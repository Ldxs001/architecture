...
- silprespec 的圈：**表象上 LLM 自主编排**——它已经足够 ReAct 的样子：每步 LLM 在场、有循环、会调用工具、有进度记录；但它绝不是 ReAct——仍是有限枚举，看似决策让 LLM 做，实则决策依旧在人这里。同一个圈，在接近人的 z 轴上往上画了一个圈。
...
| loop 形态 | for 真循环（次数固定，无嵌套） | 三层嵌套有界 loop（retry → adapter → serial） | 从平面循环到嵌套循环 |
...
- **有循环**：can_accept 判据不过就回 loop，Adapter 负责把上一步输出适配成下一步输入，serial 层遍历子任务——对应 ReAct 的循环思考-行动；
...
这个观察顺带给出了一个比特征清单更可靠的判据：**判断一个系统是不是"真 agent 化"，不看它有没有 loop、有没有工具调用、LLM 是否每步在场——这些特征 silprespec 全有；要看编排的圈画在 z 轴的哪一侧：决策让渡了没有。** 特征可以复刻，决策让渡不可伪装——伪装出来的（silprespec 这类）就是编排器长得像 agent，真让渡的才是 agent 该有的样子。母书 08a 把 Orchestrator 称为"伪 agent"，论断依据的正是分工先于名分：先看决策在哪一侧，再谈它像不像。
...
变化线的终点不是"下一个编排器"，而是一个可以反复使用的判据：**看一个系统是否真的把决策交给了 LLM，不要数它有几个 loop、调不调工具、LLM 在不在场——要看编排的圈画在 z 轴的哪一侧。** 特征会一代比一代更像 ReAct，但"决策在人"这条线只要还在，它就还是编排器；哪天这条线被跨过，才是真正的新物种。Orchestrator 与 silprespec 都还站在线的这一侧——这也是它们共同标注实验性的原因：方向已经探明，端到端的验证仍在进行，若实验收敛，本篇结论将随两篇系统文档一起更新。
...
> 说明：[1][2] 为本篇的两代实证标本（系统文档，可核验实现细节）；[3]-[9] 为母书方法论引用（本篇"决策在人/有限枚举"立论的母书依据）；[10] 为架构册导读中的代际定位与实验性声明的出处。无外部文献。

## 延伸阅读

- **架构 06 · Orchestrator 架构文档**：第 I 代标本的系统文档——本篇文章"人工编排、LLM 只做两头"的论断以其实现为实证。
- **架构 07 · silprespec-orchestrator 架构文档**：第 II 代标本的系统文档——"14 组合穷举、三层有界 loop、LLM 只填空"的机制细节见其正文。
