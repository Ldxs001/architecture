...
> 基准范本：arch-v1.0.0 成册 + arch-v1.0.1 清洗管线化补全（2026-09-02 固化）+ arch-v1.1.0 演进收束篇（架构 08）入书（2026-09-03）+ arch-v1.1.1 文意修订（z 轴正解立规 + 三组术语词汇对照，不增篇/章，2026-09-03） + arch-v1.1.2 印刷字体事故根因修复（不增篇/章，2026-09-03）+ arch-v1.1.3 Chromium page.pdf() 整档缩放根因修复（不增篇/章，2026-09-03：body 左对齐收 665/box-sizing border/表格+code+flow 标签断行/树按需 zoom 预算 665 in print media，正文回到自然 12pt；新 PDF 159 页 = 封面 + 正文 158）。
...
当前值：约 4.6 万字（出版折算 45,620），12 章（版权页 + 导读 + 架构 01-08 + 附录 A/B）。

---

## 五、版本号规范（全量维护点）

**族系骨架规范（arch-v1.2.0 固化统一；2026-09-04 起规范源上移）**：族系通用成书规范（版权页小节式六节 / 版本日志一行式书籍口径 / 附录 appendix 布局 / 99_结语标配 / 仓级文件四类判定）已于 2026-09-04 收编为母书仓根 **《成书规范.md》v1.0**（族系唯一规范源），条目不再在本文件复制。架构册仓级差异仅剩：PDF 管线在 `book/build/PDF/make_pdf.py`（与族系布局一致）、附录为《统一术语表》《运行速查》两篇。
